#!/bin/bash
  ...
  cd $LAMBDA_PATH && serverless config credentials --provider aws --key $AWS_ACCESS_KEY_ID --secret $AWS_SECRET_ACCESS_KEY --profile etherstellar && serverless deploy --stage $1
#fi;
